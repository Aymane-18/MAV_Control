...
class Autopilot:
    def __init__(self, ts_control):
        ...
        #Qlat = diag([0.001, 0.02, 0.2, 30, 1, 10.0])
        Qlat = diag([0.001, 1, 1, 1, 1, 0.1])
        Rlat = diag([1.0, 1.0])
        #Rlat = diag([3.0, 3.0])
        ...
        #Qlon = diag([10, 10, 0.001, 0.01, 40.0, 160.0, 160.0])  # u, w, q, theta, h, int_h, int_Va
        Qlon = diag([1, 1, 1, 0.01, 1, 0.1, 1])

        #Rlon = diag([1.0, 2.0])  # elevator, throttle
        Rlon = diag([0.1, 1])  # elevator, throttle
        Plon = solve_continuous_are(AAlon, BBlon, Qlon, Rlon)
        self.Klon = inv(Rlon) @ BBlon.T @ Plon

        print("LQR Longitudinal Gain Matrix Klon:\n", self.Klon)
        print("LQR Lateral Gain Matrix Klat:\n", self.Klat)
    ...
    def update(self, cmd, state):
        ...
        errorCourse = saturate(errorCourse, -np.radians(5), np.radians(5))
