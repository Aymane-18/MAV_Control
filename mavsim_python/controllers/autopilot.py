...
class Autopilot:
    def __init__(self, ts_control):
        ...
        # print("\n=== Lateral PID Gains ===")
        # print(f"Roll loop: Kp = {AP.roll_kp:.3f}, Kd = {AP.roll_kd:.3f}")
        # print(f"Course loop: Kp = {AP.course_kp:.3f}, Ki = {AP.course_ki:.3f}")
        # print(f"Yaw damper: Kr = {AP.yaw_damper_kr:.3f}, p_wo = {AP.yaw_damper_p_wo:.3f}")

        # print("\n=== Longitudinal PID Gains ===")
        # print(f"Pitch loop: Kp = {AP.pitch_kp:.3f}, Kd = {AP.pitch_kd:.3f}")
        # print(f"Altitude loop: Kp = {AP.altitude_kp:.3f}, Ki = {AP.altitude_ki:.3f}")
        # print(f"Airspeed (throttle) loop: Kp = {AP.airspeed_throttle_kp:.3f}, Ki = {AP.airspeed_throttle_ki:.3f}")
    ...
    def update(self, cmd, state):
        ...
        phi_c_unsat = cmd.phi_feedforward + self.course_from_roll.update(chi_c, state.chi)
        phi_c = self.saturate(phi_c_unsat, -np.radians(30), np.radians(30))
