...
class MavDynamics(MavDynamicsNoSensors):
    def __init__(self, Ts):
        ...
        self._P_ground = MAV.rho * MAV.gravity * (-self._state.item(2))  # pressure at takeoff
    ...
    def sensors(self):
        ...
        # ---------- Accelerometers (Eq. 7.3 or 7.4) ----------
        self._sensors.accel_x = fx / MAV.mass + MAV.gravity * np.sin(theta) + np.random.normal(0., SENSOR.accel_sigma)
        self._sensors.accel_y = fy / MAV.mass - MAV.gravity * np.cos(theta) * np.sin(phi) + np.random.normal(0., SENSOR.accel_sigma)
        self._sensors.accel_z = fz / MAV.mass - MAV.gravity * np.cos(theta) * np.cos(phi) + np.random.normal(0., SENSOR.accel_sigma)

        # ---------- Pressure Sensors (Eq. 7.9 & 7.10) ----------
        self._sensors.abs_pressure = self._P_ground \
                                    - MAV.rho * MAV.gravity * (-self._state.item(2)) \
                                    + SENSOR.abs_pres_bias \
                                    + np.random.normal(0., SENSOR.abs_pres_sigma)
        # self._sensors.abs_pressure = MAV.rho * MAV.gravity * (-self._state.item(2)) \
        #                             + SENSOR.abs_pres_bias + SENSOR.abs_pres_sigma
        self._sensors.diff_pressure = 0.5 * MAV.rho * self._Va**2 \
                                    + SENSOR.diff_pres_bias + SENSOR.diff_pres_sigma

        # ---------- GPS (Eq. 7.18–7.20 and 7.25–7.26) ----------
        if self._t_gps >= SENSOR.ts_gps:
            ...
            Vn = self._Va * np.cos(psi) + self._wind.item(0)
            Ve = self._Va * np.sin(psi) + self._wind.item(1)
